...
const CodingInterface: React.FC<CodingInterfaceProps> = ({
  ...
  const fetchQuestion = async () => {
    try {
      const response = await api.get(`/api/coding/questions/${questionId}`);
      setQuestion(response.data);
      if (response.data.supported_languages.length > 0) {
        setSelectedLanguage(response.data.supported_languages[0]);
      }
    } catch (error) {
